package com.abctreinamentos;
// Generated 16/03/2021 08:37:56 by Hibernate Tools 4.3.1

import java.math.BigDecimal;

/**
 * LogmnrcShardTsId generated by hbm2java
 */
public class LogmnrcShardTsId implements java.io.Serializable {

	private BigDecimal logmnrUid;
	private String tablespaceName;
	private BigDecimal startScn;

	public LogmnrcShardTsId() {
	}

	public LogmnrcShardTsId(BigDecimal logmnrUid, String tablespaceName, BigDecimal startScn) {
		this.logmnrUid = logmnrUid;
		this.tablespaceName = tablespaceName;
		this.startScn = startScn;
	}

	public BigDecimal getLogmnrUid() {
		return this.logmnrUid;
	}

	public void setLogmnrUid(BigDecimal logmnrUid) {
		this.logmnrUid = logmnrUid;
	}

	public String getTablespaceName() {
		return this.tablespaceName;
	}

	public void setTablespaceName(String tablespaceName) {
		this.tablespaceName = tablespaceName;
	}

	public BigDecimal getStartScn() {
		return this.startScn;
	}

	public void setStartScn(BigDecimal startScn) {
		this.startScn = startScn;
	}

	public boolean equals(Object other) {
		if ((this == other))
			return true;
		if ((other == null))
			return false;
		if (!(other instanceof LogmnrcShardTsId))
			return false;
		LogmnrcShardTsId castOther = (LogmnrcShardTsId) other;

		return ((this.getLogmnrUid() == castOther.getLogmnrUid()) || (this.getLogmnrUid() != null
				&& castOther.getLogmnrUid() != null && this.getLogmnrUid().equals(castOther.getLogmnrUid())))
				&& ((this.getTablespaceName() == castOther.getTablespaceName())
						|| (this.getTablespaceName() != null && castOther.getTablespaceName() != null
								&& this.getTablespaceName().equals(castOther.getTablespaceName())))
				&& ((this.getStartScn() == castOther.getStartScn()) || (this.getStartScn() != null
						&& castOther.getStartScn() != null && this.getStartScn().equals(castOther.getStartScn())));
	}

	public int hashCode() {
		int result = 17;

		result = 37 * result + (getLogmnrUid() == null ? 0 : this.getLogmnrUid().hashCode());
		result = 37 * result + (getTablespaceName() == null ? 0 : this.getTablespaceName().hashCode());
		result = 37 * result + (getStartScn() == null ? 0 : this.getStartScn().hashCode());
		return result;
	}

}
